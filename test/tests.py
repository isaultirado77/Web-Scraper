...
class WebScraperTest(StageTest):
    def generate(self):
        return [
            TestCase(stdin="https://icanhazdadjoke.com/j/R7UfaahVfFd",
                     check_function=self.check_valid_res),

            TestCase(stdin="https://icanhazdadjoke.com/j/asdsa",
                     check_function=self.check_not_valid_res),

            TestCase(stdin="https://icanhazdadjoke.com/j/authors",
                     check_function=self.check_not_valid_res)
        ]

    def check_valid_res(self, reply, attach=None) -> CheckResult:
        try:
            qod = requests.get("https://icanhazdadjoke.com/j/R7UfaahVfFd", headers={'Accept': 'application/json'}).json()["joke"]
        except Exception:
        ...
        if qod in reply:
            ...
            return CheckResult.wrong("Couldn't find the exact joke in the result.")
        elif isinstance(reply, (list, dict)):
            return CheckResult.wrong("Make sure you extracted the joke from the json body correctly.")
        else:
            return CheckResult.wrong("The result doesn't look like a joke... at all.")
